...
(
            SELECT
                jsonb_build_object('id', p.id, 'name', p.name, 'age', extract(year FROM age(p.date_of_birth))::int, 'height_cm', p.height_cm, 'weight_kg', p.weight_kg, 'years_of_experience', p.years_of_experience, 'neighborhood', p.neighborhood, 'fighting_style', fighting_styles.name, 'fight_type', fight_types.name, 'photos',(
                        SELECT
