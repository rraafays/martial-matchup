...
export const PhotoGrid: FC<Props> = ({
    ...
    const renderItem = (item: Item) => {
        return (
            <View key={item.key} style={{ width: itemSize, height: itemSize }}>
                {item.photo?.photo_url ? (
                    <View className="flex-1 rounded-md overflow-hidden">
                        <Image source={item.photo?.photo_url} className="flex-1 bg-black" />
                    </View>
                ) : (
                    <View className="flex-1 border border-black border-dashed rounded-md" />
                )}
            </View>
        );
    };
